...
const state = { messages: [], ttsEnabled: false, recognition: null, recognitionActive: false, currentAudio: null, currentAudioUrl: null, currentVoiceLabel: "Microsoft Edge-TTS" };
const languageMeta = {
...
async function refreshTtsStatus() { try { const res = await fetch("/api/tts-status?language=" + encodeURIComponent(languageSelect.value)); const data = await readJsonSafely(res); if (!res.ok || !data.ready) { state.currentVoiceLabel = data.voice ? "Edge-TTS · " + data.voice : "Edge-TTS 未就绪"; refreshVoiceHint(); networkStatus.textContent = data.message || "Edge-TTS 未就绪"; return; } state.currentVoiceLabel = "Edge-TTS · " + data.voice; refreshVoiceHint(); } catch { state.currentVoiceLabel = "Edge-TTS 状态未知"; refreshVoiceHint(); } }
...
async function sendMessage(content) { const trimmed = content.trim(); if (!trimmed) return; addMessage("user", trimmed); inputEl.value = ""; sendBtn.disabled = true; networkStatus.textContent = "数字人思考中"; setAvatar("thinking", "正在组织回答"); try { const res = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: modelSelect.value, language: languageSelect.value, messages: buildContextMessages() }) }); const data = await readJsonSafely(res); if (!res.ok) throw new Error(data.message || "请求失败"); const assistantMessage = data.message || "我这边暂时没组织出回答。"; addMessage("assistant", assistantMessage); networkStatus.textContent = "已连接本地 Ollama"; await speak(assistantMessage); } catch (error) { addMessage("assistant", "连接失败：" + error.message); networkStatus.textContent = "连接异常"; setAvatar("idle", "连接异常"); } finally { sendBtn.disabled = false; } }
function setupRecognition() { const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition; if (!Recognition) { listenBtn.disabled = true; listenBtn.textContent = "当前浏览器不支持语音输入"; return; } const recognition = new Recognition(); recognition.continuous = false; recognition.interimResults = true; recognition.maxAlternatives = 1; recognition.onstart = () => { state.recognitionActive = true; listenBtn.textContent = "正在听..."; setAvatar("thinking", "正在听你说话"); }; recognition.onresult = (event) => { inputEl.value = Array.from(event.results).map(r => r[0].transcript).join(""); }; recognition.onend = () => { state.recognitionActive = false; listenBtn.textContent = "语音输入"; setAvatar("idle", "待机中"); }; recognition.onerror = () => { state.recognitionActive = false; listenBtn.textContent = "语音输入"; setAvatar("idle", "语音输入失败"); }; state.recognition = recognition; }
formEl.addEventListener("submit", (e) => { e.preventDefault(); sendMessage(inputEl.value); });
languageSelect.addEventListener("change", () => { refreshLanguageHint(); refreshTtsStatus(); if (state.recognition) state.recognition.lang = languageMeta[languageSelect.value].lang; });
...
listenBtn.addEventListener("click", () => { if (!state.recognition || state.recognitionActive) return; state.recognition.lang = languageMeta[languageSelect.value].lang; try { state.recognition.start(); } catch { state.recognitionActive = false; setAvatar("idle", "语音输入失败"); } });
